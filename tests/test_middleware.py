...
def _make_context(tool_name: str = "test_tool") -> object:
    """Build a lightweight MiddlewareContext-like object for unit tests."""
    # MiddlewareContext is a frozen dataclass with many required fields.
    # We only need `.message.name` for the middleware under test, so a
    # simple namespace is sufficient and avoids coupling to internal types.

    @dataclass
    class _Ctx:
        message: FakeMessage

    return _Ctx(message=FakeMessage(name=tool_name))
